...
const decodeComponent = ({ sourceMap: url, col, line }) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!url || !col || !line) reject({ error: "All params are required" });
      const { mappings, names, sources } = await fetch(url).then((r) =>
        r.json()
      );
      if (
...
module.exports = decodeComponent;
